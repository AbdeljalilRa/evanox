                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $collection->title) }}" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" id="description" rows="6" class="form-control @error('description') is-invalid @enderror">{{ old('description', $collection->description) }}</textarea>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if($collection->image)
                                        <img src="{{ Storage::disk('s3')->temporaryUrl($collection->image, now()->addMinutes(5)) }}" style="width:80px;height:80px;margin-top:5px;object-fit:cover;border-radius:5px;">
                                    @endif
                                </div>

                                <!-- Status -->
                                <div class="mb-3 form-check form-switch">
                                    <input type="checkbox" name="is_active" id="is_active" class="form-check-input" {{ $collection->is_active ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="products[]" id="products" class="form-select @error('products') is-invalid @enderror" multiple data-placeholder="Choose products">
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" {{ $collection->products->contains($product->id) ? 'selected' : '' }}>
                                                {{ $product->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @for ($i = 1; $i <= 4; $i++)
                                    <div class="mb-3">
                                        <label for="gallery_{{ $i }}" class="form-label">Gallery Image {{ $i }}</label>
                                        <input type="file" name="gallery[]" id="gallery_{{ $i }}" class="form-control">
                                    </div>
                                @endfor
                            </div>
                        </div>
