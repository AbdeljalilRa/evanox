<nav class="w-full px-[99px] pt-[35px]">
    <div class="flex justify-between items-center">
        ...
        <div class="hidden md:flex items-center gap-[62px]">
        ...
        <div class="flex items-center gap-[41px]">
            ...
            <button class="hidden md:flex items-center text-white hover:text-gray-300 transition-colors" id="dropdown-menu-button">
    ...
    <!-- Mobile Menu -->
    <div class="md:hidden hidden transform transition-all duration-300 ease-in-out opacity-0 scale-95" id="mobile-menu">
        <div class="flex flex-col space-y-3 mt-3">
            <div class="border-b border-gray-700 pb-3">
                <a href="{{ route('store.index') }}" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-[18px]">ENTER</a>
                
                <div class="py-1">
                    <button class="text-white hover:text-gray-300 transition-colors font-montserrat font-medium flex items-center justify-between w-full text-[18px]" id="mobile-collections-button">
                        <span>COLLECTIONS</span>
                    ...
                    <div class="hidden pl-4 mt-1 space-y-1" id="mobile-collections-menu">
                        <a href="{{ route('collections') }}" class="text-gray-300 hover:text-white transition-colors py-1 block text-sm font-montserrat">View All</a>
                        @isset($navCategories)
                            @foreach($navCategories as $category)
                                <a href="{{ route('collections.show', $category->slug) }}" class="text-gray-300 hover:text-white transition-colors py-1 block text-sm font-montserrat">{{ $category->title }}</a>
                            @endforeach
                        @endisset
                    </div>
                </div>
                
                <a href="{{ route('archive') }}" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-[18px]">THE ARCHIVE</a>
                <a href="{{ route('code') }}" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-[18px]">THE CODE</a>
            </div>
            
            @auth
                <div class="border-b border-gray-700 pb-3">
                    <div class="text-white font-montserrat font-bold text-xs uppercase tracking-wider mb-2">Profile</div>
                    <a href="{{ route('profile.show') }}" class="text-gray-300 hover:text-white transition-colors py-1 block text-sm font-montserrat ml-2">My Profile</a>
                    <a href="{{ route('profile.downloads') }}" class="text-gray-300 hover:text-white transition-colors py-1 block text-sm font-montserrat ml-2">Access My Downloads</a>
                </div>
                
                <div class="pt-2">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-sm">
                            Sign Out
                        </button>
                    </form>
                </div>
            @else
                <div class="pt-2">
                    <a href="{{ route('login') }}" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-sm">Sign In</a>
                    <a href="{{ route('register') }}" class="text-white hover:text-gray-300 transition-colors py-1 font-montserrat font-medium block text-sm">Sign Up</a>
                </div>
            @endauth
        </div>
    </div>
